...
# there is a dropout layer in the generator after every convolution layer EXCEPT the last one with only 3 channels
# there is a dropout layer in the discriminator after every convolution layer EXCEPT the first one with only 3 channels
# this is because dropouts are known to create worse performance when there are not very many nodes
# https://ilyaivensky.wordpress.com/2017/04/30/dropouts-in-generator/
...
def get_model(trainable=True, load_checkpoint=False, checkpoint_dir='./checkpoints', sample_dir='./samples'):
